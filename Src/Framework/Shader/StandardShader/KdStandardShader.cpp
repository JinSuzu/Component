	...
	const Math::Color& colRate, const Math::Vector3& emissive)
{
	...
	//オブジェクト単位の情報を転送(スキンメッシュ対応)
	SetIsSkinMesh(data->IsSkinMesh());
	if (m_dirtyCBObj) 
	{
		m_cb0_Obj.Write();
	}
	...
	{
		//ノード内からのボーン情報を取得
		for (auto&& nodeIndx : data->GetBoneNodeIndices()) 
		{
			if (nodeIndx >= KdStandardShader::maxBoneBufferSize) 
			{
				assert(false && "転送できるボーンの上限数を超えました");
				return;
			}

			auto& dataNode = dataNodes[nodeIndx];
			auto& workNode = workNodes[nodeIndx];
...
#include "KdStandardShader_VS_Lit.shaderInc"
		...
		std::vector<D3D11_INPUT_ELEMENT_DESC> layout = {
			...
			{ "SKINWEIGHT", 0, DXGI_FORMAT_R32G32B32_FLOAT,		0, 56, D3D11_INPUT_PER_VERTEX_DATA, 0 }
		};
